        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                setTimeout(function () {
                    ...
                    var html = '';  //由于静态页面，所以只能作字符串拼接
                    html += '<table style="" class="layui-table" lay-even>';
                    html += '<colgroup><col width="200"><col><col width="200"><col width="100"><col width="90"><col width="90"><col width="50"><col width="50"></colgroup>';
                    html += '<thead><tr><th>发表时间</th><th>标题</th><th>链接地址</th><th>发布人</th><th>类别</th><th colspan="2">操作</th></tr></thead>';
                    html += '<tbody>';
                    //遍历资源集合
                    $.each(data, function (key, item) {
                        ...
                        html += "<td><a href='" + item.link + "'>" + item.link + "</a></td>";
                        html += "<td>" + item.user_name + "</td>";
                        html += "<td>" + item.resource_type_name + "</td>";
    ...
    var article = {
        deleteData: function (id) {
            layer.confirm('确定删除？', {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'deleteResourceRecycleApi.html',
                    data: {'id': id},
                    success: function (res) {
                        layer.close(index);
                        if (true == res) {
                            layer.msg('资源删除成功', {icon: 6});
                            location.reload();
                        } else {
                            layer.msg('资源删除失败', {icon: 5});
                        }
                    }
                });
            }, function () {
        ...
        editData: function (id) {
            layer.confirm('是否要恢复这个资源？', {
                btn: ['是', '否'] //按钮
            }, function () {
                var index = layer.load(1);
                $.ajax({
                    type: 'POST',
                    url: 'recoveryResourceRecycleApi.html',
                    data: {'id': id},
                    success: function (res) {
                        layer.close(index);
                        if (true == res) {
                            layer.msg('这个资源已经可以正常浏览', {icon: 6});
                            location.reload();
                        } else {
